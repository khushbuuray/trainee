...
class NewsletterCleanupTaskHandler extends ScheduledTaskHandler
{
        ...
        CleanupService $cleanupService    ) 
  {
    ...
    {
        return [NewsletterCleanupTask::class];
    }
    ...
    public function run(): void
    {
        ...
        $enabled = $config['IctDataCleanerPro.config.enableSchedulerOfNewsletter'] ?? false;
        $frequency = $config['IctDataCleanerPro.config.newsletterCleanupScheduleFrequency'] ?? 'weekly';

        if (!$enabled || !$this->shouldRunNow('newsletterCleanup', $frequency)) {
            return;
        }

        $this->cleanupService->runCleanup($context, false, 'scheduled', 'newsletterCleanup');

        $this->systemConfigService->set(
            'IctDataCleanerPro.config.newsletterCleanupLastRun',
