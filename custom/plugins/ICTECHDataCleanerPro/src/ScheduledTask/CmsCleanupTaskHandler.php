...
class CmsCleanupTaskHandler extends ScheduledTaskHandler
{
    ...
    public function run(): void
    {
        ...
        $enabled = $config['ICTECHDataCleanerPro.config.enableSchedulerOfCms'] ?? false;
        $frequency = $config['ICTECHDataCleanerPro.config.cmsCleanupScheduleFrequency'] ?? 'weekly';

        if (!$enabled || !$this->shouldRunNow('cmsCleanup', $frequency)) {
            return;
        }

        $this->cleanupService->runCleanup(Context::createDefaultContext(), false, 'scheduled', 'cmsCleanup');
    }
