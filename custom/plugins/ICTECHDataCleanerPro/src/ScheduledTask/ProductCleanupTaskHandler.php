...
class ProductCleanupTaskHandler extends ScheduledTaskHandler
{
    ...
    public function run(): void
    {
        ...
        $enabled = $config['ICTECHDataCleanerPro.config.enableSchedulerOfProducts'] ?? false;
        $frequency = $config['ICTECHDataCleanerPro.config.productCleanupScheduleFrequency'] ?? 'weekly';
        if (!$enabled || !$this->shouldRunNow('productCleanup', $frequency)) {
            return;
        }

         $this->cleanupService->runCleanup(Context::createDefaultContext(), false, 'scheduled', 'productCleanup');
    }
