...
class CustomerCleanupTaskHandler extends ScheduledTaskHandler
{
    ...
    public function run(): void
    {
        ...
        $enabled = $config['ICTECHDataCleanerPro.config.enableSchedulerOfCustomer'] ?? false;
        $frequency = $config['ICTECHDataCleanerPro.config.customerCleanupScheduleFrequency'] ?? 'weekly';

        if (!$enabled || !$this->shouldRunNow('customerCleanup', $frequency)) {
            return;
        }

        $this->cleanupService->runCleanup(Context::createDefaultContext(), false, 'scheduled', 'customerCleanup');
    }
